package com.synergies.synergyv2.controller;

import com.synergies.synergyv2.common.response.CommonResponse;
import com.synergies.synergyv2.model.dto.NotificationDto;
import com.synergies.synergyv2.model.entity.NotificationEntity;
import com.synergies.synergyv2.repository.NotificationRepository;
import com.synergies.synergyv2.service.NotificationService;
import io.swagger.v3.oas.annotations.Operation;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import javax.transaction.Transactional;
import java.util.List;

import static com.synergies.synergyv2.common.response.code.CommonCode.*;

@RestController
@RequiredArgsConstructor
@Slf4j
@CrossOrigin(origins = "*", allowedHeaders = "*")
@RequestMapping("/api/notification")
public class NotificationController {
    private final NotificationService notificationService;
    private final NotificationRepository notificationRepository;

    @Operation(summary = "공지사항 저장")
    @PostMapping
    @Transactional
    public ResponseEntity<CommonResponse> NotificationAdd(@RequestBody NotificationDto notificationDto) {
        log.info("NotificationAdd");
        notificationService.createNotification(notificationDto);
        return ResponseEntity.ok((CommonResponse.toResponse(OK, "저장 성공")));
    }

    @Operation(summary = "공지사항 조회")
    @GetMapping
    public ResponseEntity<CommonResponse> NotificationList() {
        log.info("NotificationList");
        List<NotificationDto> list = notificationService.getAllNotification();
        return ResponseEntity.ok((CommonResponse.toResponse(OK, list)));
    }

    @Operation(summary = "카테고리를 통한 공지사항 조회")
    @GetMapping("/{category}")
    public ResponseEntity<CommonResponse> NotificationCategoryList(@PathVariable("category") String category) {
        log.info("NotificationCategoryList");
        List<NotificationDto> list = notificationService.getAllNotificationByCategory(category);
        return ResponseEntity.ok((CommonResponse.toResponse(OK, list)));
    }

    @Operation(summary = "id로 공지사항 삭제")
    @DeleteMapping("/{id}")
    public ResponseEntity<CommonResponse> NotificationDelete(@PathVariable("id") int id) {
        log.info("NotificationDelete");
        try {
            notificationService.deleteById(id);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(CommonResponse.toErrorResponse(INTERNAL_SERVER_ERROR,"서버 에러입니다."));
        }

        return ResponseEntity.ok((CommonResponse.toResponse(OK)));
    }

    @Operation(summary = "id로 공지사항 수정")
    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity<CommonResponse> NotificationUpdate(@PathVariable("id") int id, @RequestBody NotificationDto notificationDto) {
        log.info("NotificationUpdate");
        try {
            NotificationDto notification = notificationService.getNotificationById(id);
            NotificationEntity oldEntity = notification.toNotificationEntity();
            oldEntity.updateNotification(notificationDto.getTitle(), notificationDto.getContent(), notificationDto.getCategory());
            notificationRepository.save(oldEntity);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(CommonResponse.toErrorResponse(NOT_FOUND,"존재히지 않는 Id 입니다."));
        }
        return ResponseEntity.ok((CommonResponse.toResponse(OK)));
    }
}
